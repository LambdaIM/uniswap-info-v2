...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  // if (!token1Address) {
  return `https://swap.lambda.im/#/` + (remove ? `remove` : `add`) + `/ETH`
  // } else {
  //   return (
  //     `https://swap.lambda.im/#/` +
  //     (remove ? `remove` : `add`) +
...
export function getMiningPoolLink(token0Address) {
  return `https://swap.lambda.im/#/uni/ETH/${token0Address}`
}

export function getUniswapAppLink(linkVariable) {
  let baseUniswapUrl = 'https://swap.lambda.im/#/'
  if (!linkVariable) {
...
export const urls = {
  showTransaction: (tx) => `https://scan.lambda.im/tx/${tx}/`,
  showAddress: (address) => `https://scan.lambda.im/address/${address}/`,
  showToken: (address) => `https://scan.lambda.im/token/${address}/`,
  showBlock: (block) => `https://scan.lambda.im/block/${block}/`,
}
